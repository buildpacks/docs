...
We will create a sample stack based on `Ubuntu Resolute Jellyfish`. To create a custom stack, we need to create customized build and run images.
...
Example stack IDs:
  * `io.buildpacks.stacks.resolute`
  * `io.buildpacks.stacks.resolute`
  * `io.buildpacks.samples.stacks.resolute`
  * `io.buildpacks.samples.stacks.resolute`

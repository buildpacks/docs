...
Windows image builds are now supported! See the [Windows build guide](/docs/for-app-developers/how-to/configure-build/build-for-windows)
for more details.

### Multicall launcher

When running the exported application image, it is now possible to provide arguments to pre-defined process types. 
The new flow is described [here](/docs/for-app-developers/tutorials/build-basic-app/run/#run-a-multi-process-app).

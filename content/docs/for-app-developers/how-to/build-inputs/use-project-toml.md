...
and human-readable `name` of the app we are building, and a `version`), and specify build information to pack.
...
file at the root of the app. For more on `gitignore` matching see [these docs](https://linuxize.com/post/gitignore-ignoring-files-in-git/#literal-file-names).
...
> to use different buildpacks in the preceding case, we could also call `pack build ... --buildpack ...`
...
Paste the preceding `toml` as `new-project.toml` in the `samples/apps/bash-script/` directory, and simply:
```shell script
# build the app
pack build sample-app \
    --builder cnbs/sample-builder:resolute \
